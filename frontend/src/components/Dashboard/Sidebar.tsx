...
import { Video, Shield, Settings, Home, Camera, User } from "lucide-react";
  ...
  return (
    <Link to={to}>
      ...
      >
        ...
        <span>{text}</span>
        {isActive && (
          <div className="absolute left-0 top-1/2 h-4/5 w-0.5 -translate-y-1/2 bg-primary rounded-full" />
        )}
      </Button>
    </Link>
  );
      ...
      <SidebarHeader className="pb-4 pt-6">
        <div className="flex items-center px-4">
          ...
          <h1 className="text-xl font-bold text-sidebar-foreground">
            Vigilant<span className="text-primary">Eye</span>
          </h1>
        </div>
      </SidebarHeader>
